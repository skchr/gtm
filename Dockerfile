...
# Install Nim 2.2.10 (pick arch: x86_64 → linux_x64, aarch64 → linux_arm64)
RUN ARCH_NIM="$(uname -m)" && \
    ...
    curl -sL "https://nim-lang.org/download/nim-2.2.10-linux_${NIM_ARCH}.tar.xz" -o /tmp/nim.tar.xz \
    && tar xJf /tmp/nim.tar.xz -C /opt \
    && mv /opt/nim-2.2.10 /opt/nim \
    && rm /tmp/nim.tar.xz
