...
0c. Study `src/*` with up to 50 parallel subagents to understand shared utilities & components.
...
ULTIMATE GOAL: We want to create a terminal based music player with full support. Consider missing elements and plan accordingly. If an element is missing, search first to confirm it doesn't exist, then if needed author the specification at specs/FILENAME.md. If you create a new element then document the plan to implement it in @IMPLEMENTATION_PLAN.md using a subagent. Refer to ./PROMPT.md for context at what the user wants to accomplish.
